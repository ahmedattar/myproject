...
class NEOWriter(object):
    ...
    def save_csv(self, data, filename, is_neo_object):
        if is_neo_object:
            data_list = []
            for each in data:
                data_list.append([each.name, each.nasa_jpl_url, each.absolute_magnitude_h, each.diameter_min_km, each.is_potentially_hazardous_asteroid])
            df = pd.DataFrame(data_list, columns = ['name', 'nasa_jpl_url', 'absolute_magnitude_h','diameter_min_km', 'is_potentially_hazardous_asteroid'])
